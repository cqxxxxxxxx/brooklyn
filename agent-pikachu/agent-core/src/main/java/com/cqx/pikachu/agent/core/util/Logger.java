package com.cqx.pikachu.agent.core.util;
...
public abstract class Logger {
    public static final String TTL_AGENT_LOGGER_KEY = "pikachu.agent.logger";
    ...
    public static void setLoggerImplType(String type) {
        if (loggerImplType != -1) {
            throw new IllegalStateException("Pikachu Agent logger implementation type is already set! type = " + loggerImplType);
        }
    ...
    public static Logger getLogger(Class<?> clazz) {
        if (loggerImplType == -1)
            throw new IllegalStateException("Pikachu Agent logger implementation type is NOT set!");
